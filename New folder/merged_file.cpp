...
class Table {
    ...
    public:
        ...
        void                   insert_for_update(vector<string>, vector<long long int>);          // insert record of update mode
...
void Table::insert_for_update(vector<string> tokens, vector<long long int> ids) {
    int num_fields = tokens.size() - 5;                                   // find number of records in tokens list
    // number of fileds is not correct
    if (num_fields != columns.size() - 1) {
        return;
    }
...
void Table::deletion_for_update(vector<string> tokens) {
    ...
    if (col != -1) {
        ...
        insert_for_update(tokens, ids);                                             // go to insert the updated datas 
    }
    // column not found
    else {
        return;
